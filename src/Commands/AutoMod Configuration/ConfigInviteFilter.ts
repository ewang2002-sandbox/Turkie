	...
	public async execute(client: Client, message: Message, args: string[], guildInfo: GuildInterface): Promise<void> {
		TurkieBotGuild.updateOne({ guildID: message.guild.id }, {
			...
			if (err) {
				...
				const embed: MessageEmbed = MessageFunctions.createMsgEmbed(message, `Invite Filter ${!guildInfo.moderation.inviteFilter.isEnabled ? "Enabled" : "Disabled"}`, `${!guildInfo.moderation.inviteFilter.isEnabled ? "The invite filter has been enabled." : "The invite filter has been disabled."}`);
