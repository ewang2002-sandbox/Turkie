...
export class ModerationEnforcement {
	...
	/**
	 * Issues a warning for the guild member(s) targeted. 
	 * @param {string} [reason] The reason for the warning.
	 */
	public async warnUser(reason: string = "No reason provided"): Promise<void> {
		return new Promise((resolve, reject) => {
			for (let i = 0; i < this.mem.length; i++) {
				const warningID: number = NumberFunctions.randomInt(1000000000000, 999999999999999);
				TurkieBotGuild.updateOne({ guildID: this.msg.guild.id }, {
					$push: {
						warningLogs: {
							warnedID: this.mem[i].id,
							guildID: this.msg.guild.id,
							moderatorID: this.msg.author.id,
							reason: reason,
							warningID: warningID
						}
					}
					...
					if (!err) {
						this.sendWarnUnwarnMessage(this.mem[i], reason, warningID, true);
					}
				});
			}
		});
	...
	/**
	 * Removes a warning from a user. This does NOT send a notification to the member.
	 * @param {GuildMember} member The guild member to remove the warning from. 
	 * @param {number} warningID The warning ID
	 * @param {string} [reason] The reason. 
	 */
	public async unwarnUser(member: GuildMember, warningID: number, reason: string = "No reason provided."): Promise<boolean> {
		return new Promise((resolve, reject) => {
			const warnings = this.res.warningLogs.filter(x => x.warnedID === member.id && x.warningID === warningID && x.guildID === this.msg.guild.id);
			if (!warnings) {
				return resolve(false);
			}
			TurkieBotGuild.updateOne({ guildID: this.msg.guild.id }, {
				...
				if (!err) {
					this.sendWarnUnwarnMessage(member, reason, warningID, false);
					return resolve(true);
				}
				return resolve(false);
			});
	...
	/**
	 * Sends a message to the channel where the warning was issued, the member that was warned, and the mod logs channel, if one is specified.
	 * @param {GuildMember} member The member to issue the warning to.
	 * @param {string} reason The reason for the warning.
	 * @param {number} warningID The ID of the warning.
	 * @param {boolean} warning Whether this is a warning or not.
	 */
	private sendWarnUnwarnMessage(member: GuildMember, reason: string, warningID: number, warning: boolean): void {
		if (warning) {
			const embed: RichEmbed = MessageFunctions.createMsgEmbed(this.msg,
				`**Warning From: ${this.msg.guild.name}**`, `Reason: ${reason}`,
				[{ name: "Moderator", value: `${this.msg.member} (${this.msg.author.tag})` },
				{ name: "Warning ID", value: warningID.toString() }]);
			member.send(embed).catch(e => { });
			// for mod logs and channel
			const embedForLogs: RichEmbed = MessageFunctions.createMsgEmbed(this.msg, `**Issued Warning For: ${member.user.tag}**`, `Reason: ${reason}`, [
				{ name: "Moderator", value: `${this.msg.member} (${this.msg.author.tag})` },
				{ name: "Warning ID", value: warningID.toString() }]);

			this.msg.channel.send(embedForLogs).catch(e => { });

			if (ModerationEnforcement.configuredModLogs(this.msg, this.res)) {
				(this.msg.guild.channels.get(this.res.serverConfiguration.serverLogs.modLogs.channel) as TextChannel).send(embedForLogs).catch(e => { });
			}
			return;
		} else {
			const embedForLogs: RichEmbed = MessageFunctions.createMsgEmbed(this.msg, `**Removed Warning From: ${member.user.tag}**`, `Reason: ${reason}`, [
				{ name: "Moderator", value: `${this.msg.member} (${this.msg.author.tag})` },
				{ name: "Warning ID", value: warningID.toString() }]);
